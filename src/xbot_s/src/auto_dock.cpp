...
void AutoDock::init(){
    ...
    autodock_state_pub = nh.advertise <xbot_s::AutodockState> ("state",100);
    ...
    autodock_state_msg.state = xbot_s::AutodockState::NOT_LAUNCHED;
...
//        ROS_INFO("p[1]=%d;vertical:%d;theta_diff=%f",p[1],is_vertical(),theta_diff);
    ...
    if(search_times>=60*20){
        ...
        found_docker=false;
        autodock_state_msg.state=xbot_s::AutodockState::NOT_FOUND_DOCKER;
...
void AutoDock::heading_docker(){
    ...
    while(ros::ok()&&!reached_docker){
        if(found_docker){
            autodock_state_msg.state=xbot_s::AutodockState::HEADING;
...
void AutoDock::launch_autodockCB(const std_msgs::Bool msg){
    if(msg.data == true){
        if(autodock_state_msg.state%2==0){
            ...
            autodock_state_msg.state = xbot_s::AutodockState::SEARCHING;
